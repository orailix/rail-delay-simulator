...
class BC(pl.LightningModule):
    """ 
    Behavioral Cloning (BC) implementation with a Transformer policy.

    Args:
        model_config (dict): Configuration dictionary passed to the Transformer policy,
            e.g. residual stream size, number of layers, activation function.
        ...
        sc (dict): Data scheme.
        cat (dict): Categorical features definitions.
    ...
    def compute_loss(self, outputs: torch.Tensor, labels: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        """
    ...
    def training_step(self, batch: tuple, batch_idx: int) -> torch.Tensor:
        ...
        loss = self.compute_loss(outputs, y, padding_mask)
...
def collate_fn(batch: tuple) -> tuple:
    ...
    Args:
        batch (list): List of (x, y) tensor pairs.
...
def main() -> None:
    ...
    run_path, checkpoints_path = setup_run_folder(args, model_config, 'tr_bc')
    ...
    model = BC(model_config, args.weight_decay, args.lr, dataset_config, itineraries, scheme['x'], cat, stations_emb, lines_emb, eval_config)
