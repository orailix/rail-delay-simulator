...
class Regression(pl.LightningModule):
    """ 
    Regression implementation with an MLP policy.
    ...
    def compute_loss(self, outputs: torch.Tensor, labels: torch.Tensor, loss_mask: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            outputs (torch.Tensor): Model predictions of shape (batch_size, nb_future_stations).
            labels (torch.Tensor): Ground-truth targets of shape (batch_size, nb_future_stations).
            loss_mask (torch.Tensor): Boolean mask, True for positions to ignore of shape(batch_size, nb_future_stations).
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        Returns:
            None
        """
        mae_delay, mae_horizon, counter_delay, counter_horizon, mse = compute_mae_regression_mlp(self.preds_val, self.targets_val, self.x_val, self.masks_val, self.sc, self.delay_delta_bins, self.horizon_obs_bins)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        Returns:
            None
        """
        mae_delay, mae_horizon, counter_delay, counter_horizon, mse = compute_mae_regression_mlp(self.preds_test, self.targets_test, self.x_test, self.masks_test, self.sc, self.delay_delta_bins, self.horizon_obs_bins)  
        mae = np.dot(mae_horizon, counter_horizon) / counter_horizon.sum()
...
def collate_fn(batch: tuple) -> tuple:
    ...
    Args:
        batch (list): List of (x, y, placeholder_mask) tuples.

    Returns:
        tuple: (xs, ys) where
            xs (torch.Tensor): Batched inputs.
            ys (torch.Tensor): Batched targets.
            ph_masks (torch.Tensor): Batched placeholder masks.
    """
    xs = torch.cat([item[0] for item in batch], dim=0)
    ys = torch.cat([item[1] for item in batch], dim=0)
    ph_masks = torch.cat([item[2] for item in batch], dim=0)
    
    return xs, ys, ph_masks
...
def main() -> None:
    ...
    parser.add_argument("lr", type=float, help="LR for AdamW")
    ...
    scheme_name = 'sc_reg_loc.pkl' if args.use_local_features else 'sc_reg_non.pkl'
    ...
    run_path, checkpoints_path = setup_run_folder(args, model_config, 'mlp_reg')
