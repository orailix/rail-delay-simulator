...
class DCIL(pl.LightningModule):
    """ 
    Drift Corrected Imitation Learning (DCIL) implementation with an MLP policy.
    ...
    Args:
        ...
        dataset_config (dict): Dataset configuration parameters for loading.
    ...
    def __init__(self, policy_config: dict, lr: float, weight_decay: float, dataset_config: dict, sim_config: dict, alpha: float, beta: float, itineraries: dict, sc: dict, 
                cat: dict, stations_emb: dict, lines_emb: dict, eval_config:dict, use_local_features: bool) -> None:
        super(DCIL, self).__init__()

        self.policy = MLP(**policy_config)
    ...
    def compute_loss(self, outputs: torch.Tensor, labels: torch.Tensor, distances: torch.Tensor) -> torch.Tensor:
        """
        ...
        Args:
            ...
            distances (torch.Tensor): Distance values used to weight examples of shape (batch_size).

        Returns:
            torch.Tensor: Scalar weighted loss.
        """
        distances = torch.abs(distances)
        weights = 1 / (1 + self.alpha * distances.pow(self.beta))
    
        criterion = nn.CrossEntropyLoss(reduction='none')
        per_example_loss = criterion(outputs, labels)
    ...
    def training_step(self, batch: tuple, batch_idx: int) -> torch.Tensor:
        ...
        Args:
            batch (tuple): (inputs, targets, distances).
        ...
        loss = self.compute_loss(outputs, y, distances)
...
class DataModule(pl.LightningDataModule):   
    ...
    Args:
        ...
        use_local_features (bool, optional): If True, include local features in the input. Default is False.
    ...
    def __init__(self, data_path: str, dataset_config: dict, sim_config: dict, scheme: dict, cat: dict, stations_emb: dict, lines_emb: dict, batch_size: int, 
                buffer_capacity: int, num_workers=0, pin_memory=True, eval_test=False, use_local_features = False, val_ratio=1.0) -> None:
        super().__init__()
...
def main() -> None:
    ...
    run_path, checkpoints_path = setup_run_folder(args, model_config, 'mlp_dcil')
    ...
    data_module = DataModule(args.data_path, dataset_config, sim_config, scheme, cat, stations_emb, lines_emb,args.batch_size, buffer_capacity = args.buffer_capacity, 
                num_workers=args.num_workers, eval_test=args.eval_test, use_local_features=args.use_local_features, val_ratio=args.val_ratio)
    
    model = DCIL(model_config, args.lr, args.weight_decay, dataset_config, sim_config, args.alpha, args.beta, itineraries, scheme['x'], cat, stations_emb, lines_emb, 
                eval_config, args.use_local_features)
